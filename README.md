...
[![Version](https://img.shields.io/badge/version-1.1.5-blue.svg)](package.json)
...
- **Compare two commits or refs** with `Ctrl`/`Cmd`+click, by dropping one commit on another, or from the context menu of a commit, branch or tag - the changed files appear in a panel beside the graph, with the direction swappable and both compared rows marked
