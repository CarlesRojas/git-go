...
- [ ] 🔴 **2. Expanded commit eats every keystroke.** `Graph.tsx:75-82` calls `event.preventDefault()` _before_ checking the key. While any commit is expanded, typing in inputs/dialogs/textareas loses arrow keys and any other intercepted keys. **Fix:** check the key first, only `preventDefault` for `ArrowUp`/`ArrowDown` and only when the target isn't an input/contenteditable.
...
- [ ] 🔴 **5. `setInterval(async …)` re-entrancy in auto-open.** `extension.ts:1073-1085` polls every 50ms with an async callback that itself does git work. The async function easily takes >50ms, so multiple invocations stack and `git-go.openGitGraph` can run several times. **Fix:** await `gitExtension.activate()` directly, or use `setTimeout` re-armed only after the previous attempt resolves.
